import { describe, expect, it } from "vitest";
import { planVisualRoutingFromScript } from "./visual-routing-planner.js";

describe("planVisualRoutingFromScript", () => {
  it("marks numeric cost comparison paragraphs as animation candidates", () => {
    const plan = planVisualRoutingFromScript(
      "如果你的项目要求本身就比较高，假如跑1小时GPT大概能用10块钱的话，Claude得10~40块，那按照我上个月总共大概花了4000块，这个价格我还能承受的话，Claude翻个4倍的价格我就承受不了。而恰恰GPT在编程任务方面的能力可能还会更好，很难不选择它。"
    );

    expect(plan.segments).toHaveLength(1);
    expect(plan.segments[0]).toMatchObject({
      id: "segment_01",
      source_block_ids: ["block_01"],
      boundary_action: "keep",
      visual_mode: "animation",
      animation_candidate: true,
      user_status: "pending"
    });
  });
});
