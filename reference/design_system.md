...
- 录屏成片通常会在底部叠字幕，因此页面不能只按“几何居中”排版
- 默认把真正的主视觉组整体上移，让成片里看起来仍然是居中的
- 经验做法：
  - 主视觉整体默认比数学中心上移约 `15%` 到 `20%`
  - 底部区域优先留给字幕，不放关键内容
- 如果有说明性短句，优先放顶部，不放底部
- 判断标准：页面本身看起来会略微“偏上”，但叠上字幕后刚好舒服

## 录屏控制与画面洁净度

- 录屏页默认提供键盘控制，至少保留左右方向键用于切换页面或场景
- 空格、Enter、Backspace 可以作为补充控制键，前提是与左右方向键语义一致
- 控制提示、调试注释、作者给自己的操作说明只存在于代码或交付说明里
- 最终画面不承载“按方向键翻页”“按 R 重播”“这里是翻译层说明”这类创作者视角文字
...
- 如果主体结构已经对了但仍显得偏小，可先对整组内容做一次统一缩放
- 原型页默认先按 `1.3` 做第一轮整体缩放，再决定是否继续调整
...
- 主视觉页不是绝对几何居中，而是“带字幕后看起来居中”
- 如果页面有整组主视觉（左右块 + 中间块 / 标题组 / 主图组），优先整体上移
- 如需补一句总结，默认放顶部；底部不要放希望观众必须读到的字
- 录屏时由操作者掌控翻页节奏，画面本身保持安静，不显示操作提示
...
**实现注意：**
- 如果元素本身已经用 `transform` 做定位（例如 `translate(-50%, -50%)` 居中），动画时不要直接写新的 `transform` 覆盖它
- 否则浏览器会丢掉原来的定位位移，导致元素瞬间偏移、闪跳，尤其是从第一页切到第二页时最明显
- 推荐两种安全方案：
  - 外层负责定位，内层负责动画
  - 动画里完整保留原始 `translate(...)`，只追加缩放或旋转
